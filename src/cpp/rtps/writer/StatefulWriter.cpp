        ...
        bool& activateHeartbeatPeriod)
{
    ...
    if (there_are_remote_readers_)
    {
        static constexpr uint32_t max_send_operations = 4ul;
        static constexpr uint32_t implicit_flow_controller_size =
            max_send_operations * RTPSMessageGroup::get_max_fragment_payload_size();
    ...
    if (get_seq_num_min() != SequenceNumber_t::unknown())
    {
        ...
        if (next_all_acked_notify_sequence_ <= min_low_mark)
        {
            if ( (mp_listener != nullptr) && (min_low_mark >= get_seq_num_min()))
            {
                ...
                do
                {
                    // Go to previous change, but avoid notifying changes before next_all_acked_notify_sequence_
                    --cit;
                    if ((*cit)->sequenceNumber < next_all_acked_notify_sequence_)
                    {
                        break;
                    }

                    // Notify reception of change
                    mp_listener->onWriterChangeReceivedByAll(this, *cit);

                    // Stop if we notify the first change
                } while (cit != mp_history->changesBegin());
            }
